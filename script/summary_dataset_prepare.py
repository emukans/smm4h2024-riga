    ...
    with open(tweet_data_path, 'r') as f:
        ...
        for line in reader:
            ...
            if not os.path.exists(file_location):
                if 'dev' in tweet_data_path or 'test' in tweet_data_path:
            ...
            result_json[tweet_id] = normalize_string(line[1].strip().lstrip('"').rstrip('"'))
...
if __name__ == '__main__':
    ...
    test_summary_data_path = '../data/task1/test/summary_extraction_gpt4/response'
    test_dataset_path = '../data/task1/test'

    os.makedirs(test_dataset_path, exist_ok=True)
    test_json, test_summary, test_ade, test_span = extract_summary(test_tweets_data_path, test_summary_data_path)
    ...
    for tweet_id, text in test_json.items():
        test_ade_merged[tweet_id] = normalize_string(
            text.strip().lstrip('"').rstrip('"')) + ' <sep> ' + normalize_string(
            test_ade[tweet_id].strip().lstrip('"').rstrip('"'))

    test_summary_merged = {}
    for tweet_id, text in test_json.items():
        test_summary_merged[tweet_id] = normalize_string(
            text.strip().lstrip('"').rstrip('"')) + ' <sep> ' + normalize_string(
            test_summary[tweet_id].strip().lstrip('"').rstrip('"'))


    # with open(os.path.join(test_dataset_path, 'ade_merged.json'), 'w') as f:
    #     json.dump(test_ade_merged, f)

    with open(os.path.join(test_dataset_path, 'summary.json'), 'w') as f:
